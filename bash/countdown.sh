...
#This function is defined to reset the count to maximum and tell the user that it is not allowed to interrupt the count
function interrupt {
  sleepCount=$numberOfSleeps
  echo "You interrupted the sleep count. You are not allowed to do that."
}

#This function is used to exit immediately out of the script and tell the user that the secret to get out of the script has been found.
function quit {
  echo "Ahaa. You have found the secret to get out of the script."
  exit 0
}

# Normally traps catch signals and do something useful or necessary with them

#The below command is used to trap the INT signal when received and invoke the interrupt function.
